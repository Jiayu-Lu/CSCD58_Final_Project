...
def send_payload(seq, ack, payload):
    # Send payload
    payload_packet = IP(dst=target_ip) / TCP(dport=target_port, seq=seq, ack=ack, flags='PA') / payload
    send(payload_packet)
...
def tcp_teardown(seq, ack):
    # Step 1: Send FIN
    fin_packet = IP(dst=target_ip) / TCP(dport=target_port, seq=seq, ack=ack, flags='FA')
   ...
   # fin_from_server = ... # Logic to receive server's FIN packet
    last_ack = IP(dst=target_ip) / TCP(dport=target_port, seq=fin_ack_packet[TCP].ack, ack=fin_ack_packet[TCP].seq + 1, flags='A')
    send(last_ack)

def sim():
    seq, ack = tcp_handshake()
    payload = "Hello, this is a test message."
    seq = send_payload(seq, ack, pd.create_login_data(pd.LoginStatus.SUCCESS.value | pd.LoginStatus.GUEST.value))
